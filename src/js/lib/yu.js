...
(function(w){
	...
	function div(ob){
		this.cj=function(){
			this.G_d=document.createElement('div');
			with(this.G_d.style){
				position='absolute';
				zIndex=99;
				top='0px';
			...
			this.iecss='';
			document.body.appendChild(this.G_d);
			if(typeof(ob.css)=='string'){
				if(document.all&&ob.css!='ui_box'){
				...
				if(ob.css=='ui_confirm'){
					...
					if(typeof(ob.obj)=='object'){
						...
						bd.innerHTML='<img src="?gif=ui_load.gif" alt="加载中" style="margin:10px 30px;" />';
		...
		this.ct=function(wh){
			...
			this.G_d.style.top=(h-this.G_d.offsetHeight)/2+Math.max(document.documentElement.scrollTop,document.body.scrollTop)+'px';
			this.G_d.style.left=(w-this.G_d.offsetWidth)/2+Math.max(document.documentElement.scrollLeft,document.body.scrollLeft)+'px'
		};
		this.xs=function(i){
			var T=this;
			with(this.G_d.style){
			...
			if(document.all&&i>0.9&&ob.css=='ui_box'){
		...
		this.yc=function(i){
			...
			if(typeof(ob.css)=='string'){
				if(i>0){
					...
					document.body.removeChild(this.G_d)
				}
			}
			else{
				if(i>0){
